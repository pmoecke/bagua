...
def plot_test_accuarcy(test_accuracies, epochs, algorithm):
    train_acc = np.array(test_accuracies)
    plt.plot(np.arange(1, epochs + 1), train_acc)
    plt.ylabel("Test Accuracy")
    plt.xlabel("Epochs")
    plt.title(algorithm)
    plt.savefig(f"mnist_accuracy_{algorithm}.pdf")
    plt.close()

def plot_test_error(test_errors, epochs, algorithm):
    train_acc = np.array(test_errors)
    plt.plot(np.arange(1, epochs + 1), train_acc)
    plt.ylabel("Test Loss")
    plt.xlabel("Epochs")
    plt.title(algorithm)
    plt.savefig(f"mnist_loss_{algorithm}.pdf")
    plt.close()
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        curr_acc, curr_test_loss = test(model, test_loader)
